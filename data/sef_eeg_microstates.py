...
def read_edf(filename):
    ...
    #print(header)
    # we want to print the information of the data that we read in 
    print("some info:\n\n")
    print(data.shape)
    print(data[1,:])
...
def topo(data, n_grid=64):
    ...
    """
    if data.shape[0] == 16:
        channels, locs = read_big_xyz('smaller_cap.xyz')
    elif data.shape[0] == 64:
        channels, locs = read_big_xyz('bigger_cap_64.xyz')
    else:
        channels, locs = read_big_xyz('bigger_cap.xyz')
    n_channels = len(channels)
    #locs /= np.sqrt(np.sum(locs**2,axis=1))[:,np.newaxis]
    locs /= np.linalg.norm(locs, 2, axis=1, keepdims=True)
    c = findstr('E81', channels)[0]
...
def kmeans(data, n_maps, n_runs=10, maxerr=1e-6, maxiter=500, doplot=False):
    ...
    # okay the idea is to find out where the gfp peaks and then 
    # count the number of microstates which results from this 
    print "The number of peaks: " + str(len(gfp_peaks))
